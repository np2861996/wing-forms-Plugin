( function( $ ) {
	...
	$( 'form.tag-generator-panel .control-box :input' ).change( function() {
		...
		wfp.taggen.normalize( $( this ) );
		wfp.taggen.update( $form );
	} );

	$( 'input.insert-tag' ).click( function() {
		var $form = $( this ).closest( 'form.tag-generator-panel' );
		var tag = $form.find( 'input.tag' ).val();
		wfp.taggen.insert( tag );
		tb_remove(); // close thickbox
		return false;
	} );

	wfp.taggen.update = function( $form ) {
		...
		if ( $.isFunction( wfp.taggen.update[ id ] ) ) {
			return wfp.taggen.update[ id ].call( this, $form );
		}
		...
		$form.find( 'input.tag' ).each( function() {
			...
			var components = wfp.taggen.compose( tag_type, $form );
			$( this ).val( components );
		} );
	...
	wfp.taggen.update.captcha = function( $form ) {
		var captchac = wfp.taggen.compose( 'captchac', $form );
		var captchar = wfp.taggen.compose( 'captchar', $form );

		$form.find( 'input.tag' ).val( captchac + ' ' + captchar );
	};

	wfp.taggen.compose = function( tagType, $form ) {
	...
	wfp.taggen.normalize = function( $input ) {
		var val = $input.val();
		...
		if ( $input.is( ':checkbox.exclusive' ) ) {
			wfp.taggen.exclusiveCheckbox( $input );
		}
	};

	wfp.taggen.exclusiveCheckbox = function( $cb ) {
		if ( $cb.is( ':checked' ) ) {
			$cb.siblings( ':checkbox.exclusive' ).prop( 'checked', false );
		}
	};

	wfp.taggen.insert = function( content ) {
		$( 'textarea#wfp-form' ).each( function() {
			this.focus();
