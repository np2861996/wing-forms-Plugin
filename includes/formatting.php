...
function wfp_autop( $text, $br = 1 ) {
	...
	/* wfp: take care of [response], [recaptcha], and [hidden] tag */
	...
	if ( $br ) {
		...
		$text = preg_replace_callback(
			...
			'wfp_autop_preserve_newline_callback', $text );
		$text = preg_replace( '|(?<!<br />)\s*\n|', "<br />\n", $text ); // optionally make line breaks
		$text = str_replace( '<WPPreserveNewline />', "\n", $text );

		/* wfp: remove extra <br /> just added before [response], [recaptcha], and [hidden] tags */
...
/**
 * Newline preservation help function for wfp_autop().
 *
 * @param array $matches preg_replace_callback() matches array.
 * @return string Text including newline placeholders.
 */
function wfp_autop_preserve_newline_callback( $matches ) {
 ...
 */
function wfp_sanitize_query_var( $text ) {
 ...
 */
function wfp_strip_quote_deep( $input ) {
	if ( is_string( $input ) ) {
		return wfp_strip_quote( $input );
	}
	...
	if ( is_array( $input ) ) {
		...
		foreach ( $input as $key => $text ) {
			$result[$key] = wfp_strip_quote_deep( $text );
		}
 ...
 */
function wfp_normalize_newline( $text, $to = "\n" ) {
	if ( ! is_string( $text ) ) {
		return $text;
	}
 ...
 * @return mixed Processed value.
 */
function wfp_normalize_newline_deep( $input, $to = "\n" ) {
	if ( is_array( $input ) ) {
		$result = array();

		foreach ( $input as $key => $text ) {
			$result[$key] = wfp_normalize_newline_deep( $text, $to );
		}

		return $result;
	}

	return wfp_normalize_newline( $input, $to );
}
 ...
 */
function wfp_strip_newline( $text ) {
	$text = (string) $text;
 ...
 */
function wfp_canonicalize( $text, $args = '' ) {
	// for back-compat
...
/**
 * Sanitizes Wing Forms's form unit-tag.
 *
 * @param string $tag Unit-tag.
 * @return string Sanitized unit-tag.
 */
function wfp_sanitize_unit_tag( $tag ) {
 ...
 */
function wfp_antiscript_file_name( $filename ) {
 ...
 */
function wfp_mask_password( $text, $right = 0, $left = 0 ) {
	$length = strlen( $text );
 ...
 */
function wfp_kses_allowed_html( $context = 'form' ) {
	static $allowed_tags = array();

	if ( isset( $allowed_tags[$context] ) ) {
		return apply_filters(
			'wfp_kses_allowed_html',
 ...
 */
function wfp_kses( $input, $context = 'form' ) {
	$output = wp_kses(
		$input,
		wfp_kses_allowed_html( $context )
	);
