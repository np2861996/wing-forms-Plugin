...
add_filter( 'wfp_special_mail_tags', 'wfp_special_mail_tag', 10, 4 );
 ...
 */
function wfp_special_mail_tag( $output, $name, $html, $mail_tag = null ) {
	...
	$name = preg_replace( '/^wfp\./', '_', $name ); // for back-compat
...
add_filter( 'wfp_special_mail_tags', 'wfp_post_related_smt', 10, 4 );
 ...
 * @return string Output of the given special mail-tag.
 */
function wfp_post_related_smt( $output, $name, $html, $mail_tag = null ) {
	if ( ! $mail_tag instanceof WFP_MailTag ) {
		wfp_doing_it_wrong(
			sprintf( '%s()', __FUNCTION__ ),
			__( 'The fourth parameter ($mail_tag) must be an instance of the WFP_MailTag class.', 'wing-forms' ),
...
add_filter( 'wfp_special_mail_tags', 'wfp_site_related_smt', 10, 4 );
 ...
 * @return string Output of the given special mail-tag.
 */
function wfp_site_related_smt( $output, $name, $html, $mail_tag = null ) {
	if ( ! $mail_tag instanceof WFP_MailTag ) {
		wfp_doing_it_wrong(
			sprintf( '%s()', __FUNCTION__ ),
			__( 'The fourth parameter ($mail_tag) must be an instance of the WFP_MailTag class.', 'wing-forms' ),
...
add_filter( 'wfp_special_mail_tags', 'wfp_user_related_smt', 10, 4 );
 ...
 * @return string Output of the given special mail-tag.
 */
function wfp_user_related_smt( $output, $name, $html, $mail_tag = null ) {
	if ( ! $mail_tag instanceof WFP_MailTag ) {
		wfp_doing_it_wrong(
			sprintf( '%s()', __FUNCTION__ ),
			__( 'The fourth parameter ($mail_tag) must be an instance of the WFP_MailTag class.', 'wing-forms' ),
