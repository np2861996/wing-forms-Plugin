 ...
 */
class WFP_FormTag implements ArrayAccess {
	...
	public function get_date_option( $option_name ) {
		...
		$date = apply_filters( 'wfp_form_tag_date_option',
			null,
		...
		foreach ( $options as $opt ) {
			...
			if ( 'user_' == substr( $opt, 0, 5 ) and is_user_logged_in() ) {
				...
				if ( $wing_form = WFP_WingForm::get_current() ) {
					$val = $wing_form->shortcode_attr( $this->name );
	...
	public function get_data_option( $args = '' ) {
		...
		return apply_filters( 'wfp_form_tag_data_option', null, $options, $args );
	}
