...
class WFP_Mail {
	...
	private function htmlize( $body ) {
		...
		$header = apply_filters( 'wfp_mail_html_header',
			'<!doctype html>
...
', $this );
		...
		$footer = apply_filters( 'wfp_mail_html_footer',
			'</body>
</html>', $this );
	...
	private function compose( $send = true ) {
		...
		$components = apply_filters( 'wfp_mail_components',
			$components, wfp_get_current_wing_form(), $this
		);
		...
		$subject = wfp_strip_newline( $components['subject'] );
		$sender = wfp_strip_newline( $components['sender'] );
		$recipient = wfp_strip_newline( $components['recipient'] );
		...
		if ( $this->use_html ) {
			...
			$headers .= "X-WFP-Content-Type: text/html\n";
		} else {
			$headers .= "X-WFP-Content-Type: text/plain\n";
		}
		...
		$attachments = array_filter(
			...
			function ( $attachment ) {
					...
					if ( WP_DEBUG ) {
						trigger_error(
							sprintf(
								...
								__( 'Failed to attach a file. %s is not in the allowed directory.', 'wing-forms' ),
								$path
							),
							E_USER_NOTICE
						);
				...
				if ( ! is_readable( $path ) or ! is_file( $path ) ) {
					if ( WP_DEBUG ) {
						trigger_error(
							sprintf(
								/* translators: %s: Attachment file path. */
								__( 'Failed to attach a file. %s is not a readable file.', 'wing-forms' ),
								$path
							),
							E_USER_NOTICE
						);
				...
				if ( 25 * MB_IN_BYTES < $total_size[$this->name] + $file_size ) {
					if ( WP_DEBUG ) {
						trigger_error(
							__( 'Failed to attach a file. The total file size exceeds the limit of 25 megabytes.', 'wing-forms' ),
							E_USER_NOTICE
						);
					}
	...
	public function replace_tags( $content, $args = '' ) {
		...
		return wfp_mail_replace_tags( $content, $args );
	}
...
function wfp_mail_replace_tags( $content, $args = '' ) {
	$args = wp_parse_args( $args, array(
		'html' => false,
		'exclude_blank' => false,
	) );
	...
	if ( is_array( $content ) ) {
		foreach ( $content as $key => $value ) {
			$content[$key] = wfp_mail_replace_tags( $value, $args );
		}
	...
	foreach ( $content as $num => $line ) {
		$line = new WFP_MailTaggedText( $line, $args );
...
add_action( 'phpmailer_init', 'wfp_phpmailer_init', 10, 1 );

function wfp_phpmailer_init( $phpmailer ) {
	$custom_headers = $phpmailer->getCustomHeaders();
	$phpmailer->clearCustomHeaders();
	$wfp_content_type = false;

	foreach ( (array) $custom_headers as $custom_header ) {
		$name = $custom_header[0];
		$value = $custom_header[1];

		if ( 'X-WFP-Content-Type' === $name ) {
			$wfp_content_type = trim( $value );
		} else {
			$phpmailer->addCustomHeader( $name, $value );
		}
	}

	if ( 'text/html' === $wfp_content_type ) {
		$phpmailer->msgHTML( $phpmailer->Body );
	} elseif ( 'text/plain' === $wfp_content_type ) {
		$phpmailer->AltBody = '';
	}
}

class WFP_MailTaggedText {
	...
	private function replace_tags_callback( $matches, $html = false ) {
		...
		if ( null !== $replaced ) {
			...
			$replaced = wfp_flat_join( $replaced );
		...
		if ( $form_tag = $mail_tag->corresponding_form_tag() ) {
			...
			$replaced = apply_filters(
				"wfp_mail_tag_replaced_{$type}", $replaced,
				$submitted, $html, $mail_tag
			);
		}

		$replaced = apply_filters(
			'wfp_mail_tag_replaced', $replaced,
			$submitted, $html, $mail_tag
		);
		...
		$special = apply_filters( 'wfp_special_mail_tags', null,
...
class WFP_MailTag {
	...
	public function __construct( $tag, $tagname, $values ) {
		...
		if ( ! empty( $values ) ) {
			...
			$this->values = wfp_strip_quote_deep( $matches[0] );
		}
	...
	public function corresponding_form_tag() {
		if ( $this->form_tag instanceof WFP_FormTag ) {
