 ...
 */
function wfp_plugin_path( $path = '' ) {
	return path_join( WFP_PLUGIN_DIR, trim( $path, '/' ) );
}
 ...
 * @return string URL.
 */
function wfp_plugin_url( $path = '' ) {
	$url = plugins_url( $path, WFP_PLUGIN );
...
/**
 * Include a file under WFP_PLUGIN_MODULES_DIR.
 *
 * @param string $path File path relative to the module dir.
 * @return bool True on success, false on failure.
 */
function wfp_include_module_file( $path ) {
	$dir = WFP_PLUGIN_MODULES_DIR;
 ...
 */
function wfp_upload_dir( $type = false ) {
	$uploads = wp_get_upload_dir();

	$uploads = apply_filters( 'wfp_upload_dir', array(
		'dir' => $uploads['basedir'],
		'url' => $uploads['baseurl'],
	) );
 ...
 */
function wfp_verify_nonce( $nonce, $action = 'wp_rest' ) {
 ...
 */
function wfp_create_nonce( $action = 'wp_rest' ) {
 ...
 */
function wfp_array_flatten( $input ) {
	if ( ! is_array( $input ) ) {
		return array( $input );
	}
	...
	foreach ( $input as $value ) {
		$output = array_merge( $output, wfp_array_flatten( $value ) );
	}
 ...
 */
function wfp_exclude_blank( $input ) {
 ...
 */
function wfp_support_html5() {
	return (bool) wfp_apply_filters_deprecated(
		'wfp_support_html5',
		array( true ),
		'5.6',
		''
 ...
 */
function wfp_support_html5_fallback() {
	return (bool) apply_filters( 'wfp_support_html5_fallback', false );
}


/**
 * Returns true if the Really Simple CAPTCHA plugin is used for wing forms.
 */
function wfp_use_really_simple_captcha() {
	return apply_filters( 'wfp_use_really_simple_captcha',
		WFP_USE_REALLY_SIMPLE_CAPTCHA
	);
}
 ...
 */
function wfp_validate_configuration() {
	return apply_filters( 'wfp_validate_configuration',
		WFP_VALIDATE_CONFIGURATION
	);
}


/**
 * Returns true if wfp_autop() is applied to form content.
 */
function wfp_autop_or_not() {
	return (bool) apply_filters( 'wfp_autop_or_not', WFP_AUTOP );
}
 ...
 */
function wfp_load_js() {
	return apply_filters( 'wfp_load_js', WFP_LOAD_JS );
}
 ...
 */
function wfp_load_css() {
	return apply_filters( 'wfp_load_css', WFP_LOAD_CSS );
}
 ...
 */
function wfp_link( $url, $anchor_text, $args = '' ) {
	$defaults = array(
		'id' => '',
		'class' => '',
	);
 ...
 */
function wfp_register_post_types() {
	if ( class_exists( 'WFP_WingForm' ) ) {
		WFP_WingForm::register_post_type();
		return true;
	} else {
		return false;
	}
 ...
 */
function wfp_version_grep( $version, array $input ) {
 ...
 */
function wfp_enctype_value( $enctype ) {
 ...
 */
function wfp_build_query( $args, $key = '' ) {
	$sep = '&';
	$ret = array();
	...
	foreach ( (array) $args as $k => $v ) {
		...
		if ( is_array( $v ) or is_object( $v ) ) {
			array_push( $ret, wfp_build_query( $v, $k ) );
		} else {
 ...
 */
function wfp_count_code_units( $text ) {
	static $use_mb = null;
 ...
 * @param string $version The version of Wing Forms that deprecated
 *                        the function.
 * @param string $replacement The function that should have been called.
 */
function wfp_deprecated_function( $function_name, $version, $replacement ) {
	if ( WP_DEBUG ) {
		if ( function_exists( '__' ) ) {
			trigger_error(
				sprintf(
					/* translators: 1: PHP function name, 2: version number, 3: alternative function name */
					__( 'Function %1$s is <strong>deprecated</strong> since Wing Forms version %2$s! Use %3$s instead.', 'wing-forms' ),
			...
			trigger_error(
				sprintf(
					'Function %1$s is <strong>deprecated</strong> since Wing Forms version %2$s! Use %3$s instead.',
					$function_name, $version, $replacement
				),
				E_USER_DEPRECATED
			);
 ...
 */
function wfp_apply_filters_deprecated( $hook_name, $args, $version, $replacement = '' ) {
		...
		if ( $replacement ) {
			trigger_error(
				sprintf(
					...
					__( 'Hook %1$s is <strong>deprecated</strong> since Wing Forms version %2$s! Use %3$s instead.', 'wing-forms' ),
					$hook_name,
					$version,
					$replacement
				),
			...
			trigger_error(
				sprintf(
					...
					__( 'Hook %1$s is <strong>deprecated</strong> since Wing Forms version %2$s with no alternative available.', 'wing-forms' ),
					$hook_name,
					$version
				),
				E_USER_DEPRECATED
 ...
 * @param string $version The version of Wing Forms where the message
 *                        was added.
 */
function wfp_doing_it_wrong( $function_name, $message, $version ) {
	if ( WP_DEBUG ) {
		if ( function_exists( '__' ) ) {
			if ( $version ) {
				$version = sprintf(
					/* translators: %s: Wing Forms version number. */
					__( '(This message was added in Wing Forms version %s.)', 'wing-forms' ),
					$version
				);
			}
			...
			trigger_error(
				sprintf(
					/* translators: Developer debugging message. 1: PHP function name, 2: Explanatory message, 3: Wing Forms version number. */
					__( 'Function %1$s was called incorrectly. %2$s %3$s', 'wing-forms' ),
					$function_name,
					$message,
					$version
				),
			...
			if ( $version ) {
				$version = sprintf(
					'(This message was added in Wing Forms version %s.)',
					$version
				);
			}
 ...
 */
function wfp_log_remote_request( $url, $request, $response ) {
	$log = sprintf(
		/* translators: 1: response code, 2: message, 3: body, 4: URL */
		__( 'HTTP Response: %1$s %2$s %3$s from %4$s', 'wing-forms' ),
	...
	$log = apply_filters( 'wfp_log_remote_request',
 ...
 */
function wfp_anonymize_ip_addr( $ip_addr ) {
