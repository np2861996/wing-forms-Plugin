 ...
 */
function wfp_is_email( $text ) {
	$result = is_email( $text );
	return apply_filters( 'wfp_is_email', $result, $text );
}
 ...
 */
function wfp_is_tel( $text ) {
	$text = preg_replace( '%[()/.*#\s-]+%', '', $text );
	$result = preg_match( '/^[+]?[0-9]+$/', $text );
	return apply_filters( 'wfp_is_tel', $result, $text );
}
...
function wfp_is_number( $text ) {
	...
	return apply_filters( 'wfp_is_number', $result, $text );
}
...
function wfp_is_date( $text ) {
	...
	return apply_filters( 'wfp_is_date', $result, $text );
}
 ...
 */
function wfp_is_mailbox_list( $mailbox_list ) {
 ...
 *              false if not.
 */
function wfp_is_email_in_domain( $email, $domain ) {
	$email_list = wfp_is_mailbox_list( $email );
 ...
 */
function wfp_is_email_in_site_domain( $email ) {
	if ( wfp_is_localhost() ) {
		return true;
	}
	...
	foreach ( $homes as $home ) {
		...
		if ( wfp_is_email_in_domain( $email, $sitename ) ) {
			return true;
		}
	}
 ...
 *              false otherwise.
 */
function wfp_is_file_path_in_content_dir( $path ) {
