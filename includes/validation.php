 ...
 */
class WFP_Validation implements ArrayAccess {
	 ...
	 *
	 * @param WFP_FormTag|array|string $context Context representing the
	 *                                   target field.
	 * @param WP_Error|string $error The error of the field.
	 */
	public function invalidate( $context, $error ) {
		if ( $context instanceof WFP_FormTag ) {
			$tag = $context;
		} elseif ( is_array( $context ) ) {
			$tag = new WFP_FormTag( $context );
		} elseif ( is_string( $context ) ) {
			$tags = wfp_scan_form_tags( array( 'name' => trim( $context ) ) );
			$tag = $tags ? new WFP_FormTag( $tags[0] ) : null;
		}
