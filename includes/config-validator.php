...
class WFP_ConfigValidator {
	...
	public static function get_doc_link( $error_code = '' ) {
		$url = __( 'https://github.com/np2861996/wing-forms-Plugin',
			'wing-forms'
		);
	...
	/**
	 * Returns the wing form object that is tied to this validator.
	 */
	public function wing_form() {
	...
	public function get_default_message( $code ) {
		switch ( $code ) {
			case self::error_maybe_empty:
				return __( "There is a possible empty field.", 'wing-forms' );
			case self::error_invalid_mailbox_syntax:
				return __( "Invalid mailbox syntax is used.", 'wing-forms' );
			case self::error_email_not_in_site_domain:
				return __( "Sender email address does not belong to the site domain.", 'wing-forms' );
			case self::error_html_in_message:
				return __( "HTML tags are used in a message.", 'wing-forms' );
			case self::error_multiple_controls_in_label:
				return __( "Multiple form controls are in a single label element.", 'wing-forms' );
			case self::error_invalid_mail_header:
				return __( "There are invalid mail header fields.", 'wing-forms' );
			case self::error_deprecated_settings:
				return __( "Deprecated settings are used.", 'wing-forms' );
			default:
				return '';
		}
	}
	...
	public function validate() {
		...
		do_action( 'wfp_config_validator_validate', $this );
	...
	public function save() {
		if ( $this->wing_form->initial() ) {
			return;
		}

		delete_post_meta( $this->wing_form->id(), '_config_errors' );

		if ( $this->errors ) {
			update_post_meta(
				$this->wing_form->id(), '_config_errors', $this->errors
			);
		}
	}
	...
	public function restore() {
		$config_errors = get_post_meta(
			$this->wing_form->id(), '_config_errors', true
		);
	...
	/**
	 * Callback function for WFP_MailTaggedText. Replaces mail-tags with
	...
	public function replace_mail_tags_with_minimum_input( $matches ) {
		...
		$mail_tag = new WFP_MailTag( $tag, $tagname, $values );
		...
		$form_tags = $this->wing_form->scan_form_tags(
			array( 'name' => $field_name )
		);

		if ( $form_tags ) {
			$form_tag = new WFP_FormTag( $form_tags[0] );
			...
			if ( wfp_form_tag_supports( $form_tag->type, 'selectable-values' ) ) {
				if ( $form_tag->pipes instanceof WFP_Pipes ) {
				...
				if ( $last_item and wfp_is_mailbox_list( $last_item ) ) {
			...
			// for back-compat
			$field_name = preg_replace( '/^wfp\./', '_', $field_name );
			...
			if ( '_site_admin_email' == $field_name ) {
				...
				return $this->wing_form->is_true( 'subscribers_only' )
					? $example_email
					: $example_blank;
	...
	public function detect_multiple_controls_in_label( $section, $content ) {
		...
		if ( preg_match_all( $pattern, $content, $matches ) ) {
			$form_tags_manager = WFP_FormTagsManager::get_instance();
			...
			foreach ( $matches[1] as $insidelabel ) {
				...
				foreach ( $tags as $tag ) {
					$is_multiple_controls_container = wfp_form_tag_supports(
						$tag->type, 'multiple-controls-container'
					);

					$is_zero_controls_container = wfp_form_tag_supports(
			...
			return $this->add_error( $section,
				...
				array(
					'message' =>
						...
						__( "Unavailable names (%names%) are used for form controls.", 'wing-forms' ),
	...
	public function detect_unavailable_html_elements( $section, $content ) {
		...
		if ( preg_match( $pattern, $content ) ) {
			return $this->add_error( $section,
				...
				array(
					'message' => __( "Unavailable HTML elements are used in the form template.", 'wing-forms' ),
		...
		foreach ( $tags as $tag ) {
			if ( false !== strpos( $tag->raw_name, '.' ) ) {
				return $this->add_error( $section,
					...
					array(
						'message' => __( "Dots are used in form-tag names.", 'wing-forms' ),
		...
		foreach ( $tags as $tag ) {
			if ( false !== strpos( $tag->raw_name, ':' ) ) {
				return $this->add_error( $section,
					...
					array(
						'message' => __( "Colons are used in form-tag names.", 'wing-forms' ),
		...
		foreach ( $tags as $tag ) {
			if ( $upload_max_filesize < $tag->get_limit_option() ) {
				return $this->add_error( $section,
					...
					array(
						'message' => __( "Uploadable file size exceeds PHP’s maximum acceptable size.", 'wing-forms' ),
	...
	public function validate_mail( $template = 'mail' ) {
		$components = (array) $this->wing_form->prop( $template );
		...
		$subject = new WFP_MailTaggedText(
			$components['subject'],
			array( 'callback' => $callback )
		);

		$subject = $subject->replace_tags();
		$subject = wfp_strip_newline( $subject );

		$this->detect_maybe_empty( sprintf( '%s.subject', $template ), $subject );

		$sender = new WFP_MailTaggedText(
			$components['sender'],
			array( 'callback' => $callback )
		);

		$sender = $sender->replace_tags();
		$sender = wfp_strip_newline( $sender );
		...
		if ( ! $invalid_mailbox and ! wfp_is_email_in_site_domain( $sender ) ) {
		...
		$recipient = new WFP_MailTaggedText(
			$components['recipient'],
			array( 'callback' => $callback )
		);

		$recipient = $recipient->replace_tags();
		$recipient = wfp_strip_newline( $recipient );
		...
		$additional_headers = new WFP_MailTaggedText(
			...
			if ( ! preg_match( '/^([0-9A-Za-z-]+):(.*)$/', $header, $matches ) ) {
				...
				and '' !== $header_value ) {
					$this->detect_invalid_mailbox_syntax(
						...
						array(
							'message' =>
								__( "Invalid mailbox syntax is used in the %name% field.", 'wing-forms' ),
		...
		if ( '' !== $components['attachments'] ) {
			...
			if ( $max < $total_size ) {
				$this->add_error( sprintf( '%s.attachments', $template ),
					...
					array(
						'message' => __( "The total size of attachment files is too large.", 'wing-forms' ),
	...
	public function detect_invalid_mailbox_syntax( $section, $content, $args = '' ) {
		...
		if ( ! wfp_is_mailbox_list( $content ) ) {
			return $this->add_error( $section,
	...
	public function detect_file_not_found( $section, $content ) {
		...
		if ( ! is_readable( $path ) or ! is_file( $path ) ) {
			return $this->add_error( $section,
				...
				array(
					'message' =>
						__( "Attachment file does not exist at %path%.", 'wing-forms' ),
	...
	public function detect_file_not_in_content_dir( $section, $content ) {
		...
		if ( ! wfp_is_file_path_in_content_dir( $path ) ) {
			return $this->add_error( $section,
				self::error_file_not_in_content_dir,
				array(
					'message' =>
						__( "It is not allowed to use files outside the wp-content directory.", 'wing-forms' ),
	...
	public function validate_messages() {
		$messages = (array) $this->wing_form->prop( 'messages' );
	...
	public function validate_additional_settings() {
		$deprecated_settings_used =
			$this->wing_form->additional_setting( 'on_sent_ok' ) ||
			$this->wing_form->additional_setting( 'on_submit' );
