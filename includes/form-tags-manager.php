...
/**
 * Wrapper function of WFP_FormTagsManager::add().
 */
function wfp_add_form_tag( $tag_types, $callback, $features = '' ) {
...
/**
 * Wrapper function of WFP_FormTagsManager::remove().
 */
function wfp_remove_form_tag( $tag_type ) {
	$manager = WFP_FormTagsManager::get_instance();
...
/**
 * Wrapper function of WFP_FormTagsManager::replace_all().
 */
function wfp_replace_all_form_tags( $content ) {
	$manager = WFP_FormTagsManager::get_instance();
...
/**
 * Wrapper function of WFP_WingForm::scan_form_tags().
 */
function wfp_scan_form_tags( $cond = null ) {
	$wing_form = WFP_WingForm::get_current();

	if ( $wing_form ) {
		return $wing_form->scan_form_tags( $cond );
	}
...
/**
 * Wrapper function of WFP_FormTagsManager::tag_type_supports().
 */
function wfp_form_tag_supports( $tag_type, $feature ) {
	$manager = WFP_FormTagsManager::get_instance();
...
class WFP_FormTagsManager {
	 ...
	 *
	 * @return WFP_FormTagsManager The singleton manager.
	 */
	 ...
	 *
	 * @return array Array of WFP_FormTag objects.
	 */
	...
	private function scan_callback( $matches, $replace = false ) {
		...
		if ( is_array( $attr ) ) {
			if ( is_array( $attr['options'] ) ) {
				...
				and ! empty( $attr['options'] ) ) {
					...
					if ( ! wfp_is_name( $scanned_tag['raw_name'] ) ) {
			...
			if ( WFP_USE_PIPE ) {
				$pipes = new WFP_Pipes( $scanned_tag['raw_values'] );
		...
		$scanned_tag = apply_filters( 'wfp_form_tag', $scanned_tag, $replace );

		$scanned_tag = new WFP_FormTag( $scanned_tag );
	...
	private function parse_atts( $text ) {
		...
		if ( preg_match( $pattern, $text, $matches ) ) {
			...
			if ( ! empty( $matches[2] ) ) {
				...
				$atts['values'] = wfp_strip_quote_deep( $matched_values[0] );
			}
		} else {
			$atts = $text;
		}
