...
class WFP_ConstantWing_WingPostRequest {
	...
	public function build( WFP_Submission $submission ) {
		$this->set_create_source( 'Wing' );
		...
		$wing_lists = isset( $service_option['wing_lists'] )
			? $service_option['wing_lists'] : array();
		...
		and ! empty( $prop['wing_lists'] ) ) {
			foreach ( (array) $prop['wing_lists'] as $list_id ) {
	...
	public function set_create_source( $create_source ) {
		if ( ! in_array( $create_source, array( 'Wing', 'Account' ) ) ) {
			return false;
		}
	...
	protected function strlen( $text ) {
		return wfp_count_code_units( $text );
	}
