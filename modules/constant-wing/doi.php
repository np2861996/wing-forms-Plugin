...
add_action(
	'doihelper_init',
	'wfp_constant_wing_doi_register_agent',
	10, 0
);

/**
 * Registers wfp_constant_wing as an agent.
 */
function wfp_constant_wing_doi_register_agent() {
	if ( ! function_exists( 'doihelper_register_agent' ) ) {
		return;
	}

	doihelper_register_agent( 'wfp_constant_wing', array(
		'optin_callback' => apply_filters(
			'wfp_constant_wing_doi_optin_callback',
			'wfp_constant_wing_doi_default_optin_callback'
		),
		'email_callback' => apply_filters(
			'wfp_constant_wing_doi_email_callback',
			'wfp_constant_wing_doi_default_email_callback'
		),
	) );
}
 ...
 */
function wfp_constant_wing_doi_default_optin_callback( $properties ) {
	$service = WFP_ConstantWing::get_instance();

	if ( $service->is_active() ) {
		$service->create_wing( $properties );
	}
}
 ...
 */
function wfp_constant_wing_doi_default_email_callback( $args ) {
